...
| D2 | Kestrel uses six command cycles and one bounded adversary. | Small enough for rapid testing; enough for ownership, reaction, doubt, payoff, pain and reckoning. |
| D3 | Kestrel/Ravellan are fictional prototype content. | Mechanically testable without importing real-faction constraints or expanding breadth. |
| D4 | Prototype starts at two normal interventions per cycle. | Creates scarcity without claiming final balance. |
| D5 | Lattice unlocks named Task Collection, not a numeric bonus. | Capability changes action/information space. |
| D6 | The V1 packet loop is not a protected product invariant. | Human play falsified “polish the old loop” as the current path to fun. |
| D7 | Delegate executes the responsible officer's recommendation; other chiefs may visibly dissent. | Command by exception without a chief-selection bypass. |
