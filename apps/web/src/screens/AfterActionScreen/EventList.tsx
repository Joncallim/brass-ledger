...
export function EventList({ events }: Props) {
  return (
    <section>
      ...
      {events.length === 0 ? (
        <p className="text-sm text-ink/40">No events matured this month.</p>
      ) : (
        <div className="space-y-2">
          {events.map((event) => (
            <div key={event.id} className="border border-yellow-800/60 bg-yellow-950/40 px-4 py-3">
              <p className="text-sm font-semibold text-yellow-300 mb-1">{event.doctrineTrigger ? "Doctrine consequence" : "Event"}: {event.title}</p>
